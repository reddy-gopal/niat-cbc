...
export default function BootcampReelGenerator({ copy, photos }: Props) {
  ...
  const handleShareAll = useCallback(async () => {
    ...
    // Not supported — show the panel so user picks manually
  ...
  const handleShareWhatsApp = useCallback(async () => {
    ...
    // Try native share targeting WhatsApp on mobile
    if (typeof navigator.canShare === 'function' && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({ files: [file], title: 'My NIAT Bootcamp 2026 Reel', text: shareText });
        setShowSharePanel(false);
    ...
    const blobUrl = URL.createObjectURL(stRef.current.recBlob!);
          ...
          {showDownload && (
            <div className="flex flex-col gap-2 w-full">
              <div className="flex gap-3 w-full">
                ...
                <button
                  type="button"
                  onClick={() => { setShowSharePanel(p => !p); handleShareAll(); }}
              ...
              {/* Share options panel */}
              {showSharePanel && (
                <div className="w-full bg-[#1a1a1a] border border-[#333] rounded-xl p-3 flex flex-col gap-2">
                  <p className="text-[#aaa] text-xs text-center mb-1">Share your reel</p>

                  {/* WhatsApp */}
                  <button
                    type="button"
                    onClick={handleShareWhatsApp}
                    className="flex items-center gap-3 px-4 py-2.5 rounded-lg bg-[#25D366] text-white font-semibold text-sm hover:opacity-85 transition"
                  >
                  ...
                  <button
                    type="button"
                    onClick={handleDownload}
                    className="flex items-center gap-3 px-4 py-2.5 rounded-lg bg-gradient-to-r from-[#833ab4] via-[#fd1d1d] to-[#fcb045] text-white font-semibold text-sm hover:opacity-85 transition"
                  >
                  ...
                  <button
                    ...
                    className="text-[#666] text-xs text-center py-1 hover:text-[#aaa] transition"
                  >
                    Cancel
                  </button>
                </div>
