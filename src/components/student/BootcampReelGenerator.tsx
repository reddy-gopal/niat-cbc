...
export default function BootcampReelGenerator({ copy, photos, stonesEarned = 0 }: Props) {
  ...
  const handleShare = useCallback(async () => {
    ...
    const file = new File([st.recBlob], fileName, { type: 'video/mp4' });
    const shareText = 'I just created my personalized NIAT Bootcamp 2026 reel! #NIATBootcamp2026';

    // Native share sheet — shows every app on the device (WhatsApp, Instagram, etc.)
    ...
    // Fallback for desktop browsers that don't support file sharing: just download
    const blobUrl = URL.createObjectURL(st.recBlob);
